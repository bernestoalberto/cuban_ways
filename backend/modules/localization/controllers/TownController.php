...
use yii\web\View;
use backend\modules\localization\models\Town;
use backend\modules\localization\models\TownSearch;
...
/** 
 * townimplenta las acciones del modelo town .
 *
 */
class TownController extends Controller
{
 ...
 /**
    ...
    public function actionIndex()
    {
		...
		$this->view->registerJsFile('@web/js/cuban_ways_db_js/localization/town/kendo_town_datasource.js',
				['depends' => [AppAssetMake::className()],'position'=>View::POS_END]);
        $this->view->registerJsFile('@web/js/cuban_ways_db_js/localization/town/kendo_town_components.js',
            ['depends' => [AppAssetMake::className()],'position'=>View::POS_END]);
		$this->view->registerJsFile('@web/js/cuban_ways_db_js/localization/town/kendo_town_actions_ajax.js',
				['depends' => [AppAssetMake::className()],'position'=>View::POS_END]);
        $this->view->registerJsFile('@web/js/cuban_ways_db_js/localization/town/kendo_town_init.js',
				['depends' => [AppAssetMake::className()],'position'=>View::POS_END]);
 ...
 /**
    ...
    public function actionLoad()
    {
		...
		array_push($this->view->js,Yii::$app->homeUrl.'/js/cuban_ways_db_js/localization/town/kendo_town_components.js');
		array_push($this->view->js,Yii::$app->homeUrl.'/js/cuban_ways_db_js/localization/town/kendo_town_actions_ajax.js');
		array_push($this->view->js,Yii::$app->homeUrl.'/js/cuban_ways_db_js/localization/town/kendo_town_init.js');
        return $this->getView()->render('@backend/modules/localization/views/town/index',array('index'=>false));
    }
/**
	 ...
	 else
	 	{
            ...
            if ($model->load(Yii::$app->request->post())) {
                try {
                    	...
                    	$result=$model->save();
                    if($result){
                       return $this->redirect(Yii::$app->homeUrl.'localization/town', [
...
/**
    ...
    public function actionUpdate()
    {
        ...
        if (Yii::$app->request->getIsAjax() ) {
			...
			$town= (object)Yii::$app->request->post('Town');
            $model =  $this->findModel($town->id_town);
            if ($model->load(Yii::$app->request->post())) {
                try {
                    	$result=$model->update();
                        $jsonResult = array('success' => true, 'message' => '');
		...
		return $this->render('form', [
			...
			$id_town=Yii::$app->request->post('Town')['id_town'];
					$condition =array(
						'id_town'=>$id_town
						);
			$model=Town::findOne($condition);
    ...
    public function actionDelete()
    {
        if($_POST['array']){
            ...
            foreach ($array as $town) {
                    ...
                    $jsonResult = array('success' => true, 'message' => 'El town  fue eliminado con exito');
                }            }
			catch (Exception $e) {
			...
			if(isset($_GET['combo']))
			{
				...
				if(isset($_GET['q']))
					$like=$_GET['q'];
		 			$rows = $query
		 				->from('town')
		 				->orderBy(array('town.id_town'=>SORT_ASC))
			 	->join('inner join','province','town.id_province=province.id_province')
			 	->select('town.*,province.name_province ,town.name_town as text ,town.id_town as id ')
			 	->where('town.name_town LIKE '."'%".$like."%'")
			 	->all();
				$result['data']=$rows;
				$rows=$result;
			}
		 	...
		 	$rows = $query
		 		...
		 		->orderBy(array('town.id_town'=>SORT_ASC))
			 	->join('inner join','province','town.id_province=province.id_province')
			 	->select('town.*,province.name_province ')
			 	->all();
...
/**
     * Load  from Excel a list of the Town table based on its primary key value.
		...
		if (Yii::$app->request->getIsAjax()) {
			...
			foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) 
			{
				$lastRow = $worksheet->getHighestRow();
					...
					{
					$id_town = $worksheet->getCell('A' . $row)->getFormattedValue();
					if (!empty($id_town)) {
						$id_town = $worksheet->getCell('A' . $row)->getValue();
						$name_town = $worksheet->getCell('B' . $row)->getValue();
						$id_province = $worksheet->getCell('C' . $row)->getValue();
						$town = new Town();
						$town->id_town  =  $id_town;
						$town->name_town  =  $name_town;
						$town->id_province  =  $id_province;
						$town->save();
					}
					}
				}
...
/**
     * Create a Excel file  from a list of the Town 
    ...
    public  function actionExcel()
    {
			...
			$townlist=json_decode($_POST['town_export']);
        	...
        	$name_user=Yii::$app->getUser()->identity->oldAttributes['username'];
				...
				->setTitle('Listado de Town')
				->setSubject('Listado de Town')
				->setDescription('Documento con el listado de Towns segun '.Yii::$app->id);
        		$objPHPExcel->setActiveSheetIndex(0);
				$objPHPExcel->getActiveSheet()->setCellValue('A1', 'id_town');
				$objPHPExcel->getActiveSheet()->setCellValue('B1', 'name_town');
				$objPHPExcel->getActiveSheet()->setCellValue('C1', 'id_province');
				$i=2;
				foreach($townlist as  $town) 
					{	
						$town=(object)$town;
						$objPHPExcel->getActiveSheet()->setCellValue('A'.$i, $town->id_town);
						$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, $town->name_town);
						$objPHPExcel->getActiveSheet()->setCellValue('C'.$i, $town->id_province);
						$i++;
					}

	 			$exceklib= new \PHPExcel_IOFactory();
        		$objWriter = $exceklib->createWriter($objPHPExcel, 'Excel2007');
				header("Content-Disposition: attachment; filename=\"Listado_town.xls\"");
...
/**
	 * Find a single Town model.
	 * @return mixed
	 */
public function actionFindbyukjson()
	{
		if (Yii::$app->request->getIsAjax() && isset( $_GET['Town'])) {
			\Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
			$condition=$_GET['Town'];
			$town=Town::findByUK($condition);
			$jsonResult = array('valid' => true);
			if($town!=null &&  $_GET['id_town']!=$town['id_town'])
