...
use yii\web\View;
use backend\modules\security\models\Role;
use backend\modules\security\models\RoleSearch;
...
/** 
 * roleimplenta las acciones del modelo role .
 *
 */
class RoleController extends Controller
{
 ...
 /**
    ...
    public function actionIndex()
    {
		...
		$this->view->registerJsFile('@web/js/cuban_ways_db_js/security/role/kendo_role_datasource.js',
				['depends' => [AppAssetMake::className()],'position'=>View::POS_END]);
        $this->view->registerJsFile('@web/js/cuban_ways_db_js/security/role/kendo_role_components.js',
            ['depends' => [AppAssetMake::className()],'position'=>View::POS_END]);
		$this->view->registerJsFile('@web/js/cuban_ways_db_js/security/role/kendo_role_actions_ajax.js',
				['depends' => [AppAssetMake::className()],'position'=>View::POS_END]);
        $this->view->registerJsFile('@web/js/cuban_ways_db_js/security/role/kendo_role_init.js',
				['depends' => [AppAssetMake::className()],'position'=>View::POS_END]);
 ...
 /**
    ...
    public function actionLoad()
    {
		...
		array_push($this->view->js,Yii::$app->homeUrl.'/js/cuban_ways_db_js/security/role/kendo_role_components.js');
		array_push($this->view->js,Yii::$app->homeUrl.'/js/cuban_ways_db_js/security/role/kendo_role_actions_ajax.js');
		array_push($this->view->js,Yii::$app->homeUrl.'/js/cuban_ways_db_js/security/role/kendo_role_init.js');
        return $this->getView()->render('@backend/modules/security/views/role/index',array('index'=>false));
    }
/**
	 ...
	 else
	 	{
            ...
            if ($model->load(Yii::$app->request->post())) {
                try {
                    	...
                    	$result=$model->save();
                    if($result){
                       return $this->redirect(Yii::$app->homeUrl.'security/role', [
...
/**
    ...
    public function actionUpdate()
    {
        ...
        if (Yii::$app->request->getIsAjax() ) {
			...
			$role= (object)Yii::$app->request->post('Role');
            $model =  $this->findModel($role->id_role);
            if ($model->load(Yii::$app->request->post())) {
                try {
                    	$result=$model->update();
                        $jsonResult = array('success' => true, 'message' => '');
		...
		return $this->render('form', [
			...
			$id_role=Yii::$app->request->post('Role')['id_role'];
					$condition =array(
						'id_role'=>$id_role
						);
			$model=Role::findOne($condition);
    ...
    public function actionDelete()
    {
        if($_POST['array']){
            ...
            foreach ($array as $role) {
                    ...
                    $jsonResult = array('success' => true, 'message' => 'El role  fue eliminado con exito');
                }            }
			catch (Exception $e) {
			...
			if(isset($_GET['combo']))
			{
				...
				if(isset($_GET['q']))
					$like=$_GET['q'];
		 			$rows = $query
		 				->from('role')
		 				->orderBy(array('role.id_role'=>SORT_ASC))
			 	->select('role.*,role.name_role as text ,role.id_role as id ')
			 	->where('role.name_role LIKE '."'%".$like."%'")
			 	->all();
				$result['data']=$rows;
				$rows=$result;
			}
...
/**
     * Load  from Excel a list of the Role table based on its primary key value.
		...
		if (Yii::$app->request->getIsAjax()) {
			...
			foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) 
			{
				$lastRow = $worksheet->getHighestRow();
					...
					{
					$id_role = $worksheet->getCell('A' . $row)->getFormattedValue();
					if (!empty($id_role)) {
						$id_role = $worksheet->getCell('A' . $row)->getValue();
						$name_role = $worksheet->getCell('B' . $row)->getValue();
						$role = new Role();
...
/**
     * Create a Excel file  from a list of the Role 
    ...
    public  function actionExcel()
    {
			...
			$rolelist=json_decode($_POST['role_export']);
        	...
        	$name_user=Yii::$app->getUser()->identity->oldAttributes['username'];
				...
				->setTitle('Listado de Role')
				->setSubject('Listado de Role')
				->setDescription('Documento con el listado de Roles segun '.Yii::$app->id);
        		$objPHPExcel->setActiveSheetIndex(0);
				$objPHPExcel->getActiveSheet()->setCellValue('A1', 'id_role');
				$objPHPExcel->getActiveSheet()->setCellValue('B1', 'name_role');
				$i=2;
				foreach($rolelist as  $role) 
					{	
						$role=(object)$role;
						$objPHPExcel->getActiveSheet()->setCellValue('A'.$i, $role->id_role);
						$objPHPExcel->getActiveSheet()->setCellValue('B'.$i, $role->name_role);
						$i++;
					}

	 			$exceklib= new \PHPExcel_IOFactory();
        		$objWriter = $exceklib->createWriter($objPHPExcel, 'Excel2007');
				header("Content-Disposition: attachment; filename=\"Listado_role.xls\"");
...
/**
	 * Find a single Role model.
	 * @return mixed
	 */
public function actionFindbyukjson()
	{
		if (Yii::$app->request->getIsAjax() && isset( $_GET['Role'])) {
			\Yii::$app->response->format = \yii\web\Response::FORMAT_JSON;
			$condition=$_GET['Role'];
			$role=Role::findByUK($condition);
			$jsonResult = array('valid' => true);
			if($role!=null &&  $_GET['id_role']!=$role['id_role'])
