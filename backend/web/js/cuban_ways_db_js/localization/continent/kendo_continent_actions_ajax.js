...
$("#btn_modal_continent").click(function() {
	Continent.oldElement=null;
    Continent.continent_form.data('bootstrapValidator').resetForm(true);
    $('#continent_form')[0].reset();
	 $("#title_continent").html('Insertar Continent');
	 $("#taskcontinent").val('insert');
    $("#modal_continent").modal();
	});
...
function showUpdate_continent(e) {

    Continent.continent_form.data('bootstrapValidator').resetForm(true);
	 var dataItem=Continent.finditem(e.id);
	 Continent.oldElement= dataItem;
	 $('#id_continent').val(dataItem.id_continent);
	 	$("#id_continent").val(Continent.oldElement.id_continent);
	 	$("#name_continent").val(Continent.oldElement.name_continent);
	 $("#taskcontinent").val('update');
	 $("#title_continent").html('Actualizar Continent');
	 $("#modal_continent").modal();
	}


//Boton Accion
$('#btnaction_continent').click(function () {
    Continent.continent_form.data('bootstrapValidator').validate();
        if(Continent.continent_form.data('bootstrapValidator').isValid())
			{
       		var continent={};
       		continent.name_continent=$("#name_continent").val();
       		var url=urlhome+"localization/continent/create";
        	if($('#taskcontinent').val()=="update")
        		{
            		var url=urlhome+"localization/continent/update";
            		continent.id_continent=Continent.oldElement.id_continent;
        		}
        $('body').modalmanager('loading');
         $.ajax({
            type: "POST",
            ...
            success:function(response){
                if(response.success)
                {
                    ...
                    img=urlhome+"../themes/make/assets/notifications/img/ok.png ",
                        NotificationW8('#2D8F3C',message,'El continent fue '+accion+' correctamente' ,5000,img)
                }
                else
                {
