...
$("#btn_modal_permission_role").click(function() {
    ...
    $('#permission_role_form')[0].reset();
	 $("#title_permission_role").html('Insertar Permission_role');
	 $("#taskpermission_role").val('insert');
	$('#select2-id_permission_combo-container').html('');
	$('#select2-id_role_combo-container').html('');
    $("#modal_permission_role").modal();
	});
...
function showUpdate_permission_role(e) {

    Permission_role.permission_role_form.data('bootstrapValidator').resetForm(true);
	 var dataItem=Permission_role.finditem(e.id);
	 Permission_role.oldElement= dataItem;
	 $('#id_permission_role').val(dataItem.id_permission_role);
	 	$("#id_permission_role").val(Permission_role.oldElement.id_permission_role);
	 	Permission_role.combobox_permission.append('<option selected="" value="'+Permission_role.oldElement.id_permission+'">'+Permission_role.oldElement.name_permission+'</option>').change();
	 	Permission_role.combobox_permission.val(Permission_role.oldElement.id_permission);
	 	Permission_role.combobox_role.append('<option selected="" value="'+Permission_role.oldElement.id_role+'">'+Permission_role.oldElement.name_role+'</option>').change();
	 	Permission_role.combobox_role.val(Permission_role.oldElement.id_role);
	 $("#taskpermission_role").val('update');
	 $("#title_permission_role").html('Actualizar Permission_role');
	 $("#modal_permission_role").modal();
	}


//Boton Accion
$('#btnaction_permission_role').click(function () {
    Permission_role.permission_role_form.data('bootstrapValidator').validate();
        if(Permission_role.permission_role_form.data('bootstrapValidator').isValid())
			{
       		var permission_role={};
       		permission_role.id_permission=Permission_role.combobox_permission.val();
       		permission_role.id_role=Permission_role.combobox_role.val();
       		var url=urlhome+"security/permission_role/create";
        	if($('#taskpermission_role').val()=="update")
        		{
            		var url=urlhome+"security/permission_role/update";
            		permission_role.id_permission_role=Permission_role.oldElement.id_permission_role;
        		}
        $('body').modalmanager('loading');
         $.ajax({
            type: "POST",
            ...
            success:function(response){
                if(response.success)
                {
                    ...
                    img=urlhome+"../themes/make/assets/notifications/img/ok.png ",
                        NotificationW8('#2D8F3C',message,'El permission_role fue '+accion+' correctamente' ,5000,img)
                }
                else
                {
                ...
                Permission_role.gridDataSource.read();
                $('body').modalmanager('removeLoading');
                $("#modal_permission_role").modal('hide');
            },
